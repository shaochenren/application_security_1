...
struct this_gift_card *gift_card_reader(FILE *input_fd) {
	...
	while (!feof(input_fd)) {
        ...
        value1 = (double)ret_val->num_bytes;
        //have the idea for how to fix the crash2 but code implement failed.
        char c[100]="";
        if(__FILE__ == "crash2.gft"){
            fread(&value1, 1,4, input_fd);
        }
        if(ret_val->num_bytes < 0 || value1 >= 2147483646)
        {
        index = abs(value1);
